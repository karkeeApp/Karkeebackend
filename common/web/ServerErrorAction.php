 ...
 * @since 2.0
 */
class ServerErrorAction extends Action
{
    /**
    ...
    protected function renderHtmlResponse()
    {
        ...
        if (isset($trace[0]) AND isset($trace[0]['args'][0]) AND method_exists($trace[0]['args'][0], 'getPathInfo') AND preg_match("/^server\//", $trace[0]['args'][0]->getPathInfo())) {
            return $this->controller->render("errorServer", $this->getViewRenderParams());
        } elseif(isset($trace[0]) AND isset($trace[0]['class']) AND preg_match("/controllers\\\server/", $trace[0]['class']) ) {
            return $this->controller->render("errorServer", $this->getViewRenderParams());
        } else {
