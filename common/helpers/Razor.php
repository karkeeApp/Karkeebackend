...
class Razor extends BaseViewRenderer
{
    private $_input;
    private $_output;
    private $_sourceFile;
    ...
    public function generateViewStringFile($name, $content='', $viewFile, $return = FALSE)
    {
    	$this->_sourceFile = $name;
        $this->_input = $content;
        $this->_output = "<?php /* source file: {$name} */ ?>\n";

        $this->parse(0, strlen($this->_input));

        if ($return) return $this->_output;

        file_put_contents($viewFile, $this->_output);
    }

    public function renderString($name, $content='', $params = [])
	{
        $this->_layoutView = $name;

        $runtime = Yii::getAlias('@runtime');

        $relpath = md5($name).'.php';
        // $relpath = str_replace( str_replace('\runtime', '', $runtime) .'\\' , '', $file);
        $directory = $runtime.'/razor';
        $newfile   = $directory.'/'.$relpath;
 
        FileHelper::createDirectory( $directory );
        self::generateViewStringFile($name, $content, $newfile);
        extract($params);
