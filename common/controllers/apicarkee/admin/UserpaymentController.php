...
class UserpaymentController extends Controller
{
    ...
    public function actionCreate(){
        ...
        if (!empty($_FILES['screenshot']) AND !is_null($_FILES['screenshot']) AND count($_FILES['screenshot']) > 0) $form->file = UploadedFile::getInstanceByName('screenshot');
        if (!is_null($form->file) AND count($_FILES['screenshot']) > 0) $form->filename = hash('crc32', $form->file->name) . time() . '.' . $form->file->extension;
