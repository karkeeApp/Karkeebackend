...
class FileController extends Controller
{
    ...
    public function actionAds($id=0) {
        ...
        static::resizing(Yii::$app->params['dir_ads'], $file->image);
    }
    ...
    public function actionPayment($id=0) {
        ...
        static::resizing(Yii::$app->params['dir_payment'], $file->filename);
    }

    public function actionSponsor($id=0){
        $file = Sponsor::findOne($id);

        if (!$file) {
            throw new \yii\web\HttpException(404, 'File not found.');
        }
        static::resizing(Yii::$app->params['dir_sponsor'], $file->filename);
    }
}