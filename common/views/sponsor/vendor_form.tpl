...
<div class="hide">
    ...
    <input type="hidden" name="action" value="@(($userForm->user_id) ? 'edit' : 'add_sponsor')">
</div>
@$form->field($userForm, 'role')->dropDownList(['' => 'Select role'] + User::roles(), ['class' => 'form-control input-sm'])
    ...
    (function($) {
        ...
        $('#save')
            .bind('click', function(e) {
                ...
                serverProcess({
                    @if(isset($user)) {
                    action:'sponsor/edit-sponsor',
                        data: $('#user-form').serialize() + '&user_id=@($user->user_id)',
                } else {
                    action:'sponsor/add-sponsor',
